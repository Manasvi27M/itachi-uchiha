...
const ResumePreviewStandardTemplate = forwardRef(({
  basicDetails,
  education,
  technicalExperience,
  skills,
  ...
  // ← Define renderDescription inside the function scope
  ...
  return (
    <div
      ref={ref}
      className='h-full bg-slate-100 w-full rounded-xl flex flex-col gap-[3px] text-neutral-800 p-5'
    >
      ...
      <div className='flex text-[8px] flex-col'>
        <h1 className='text-[12px] text-teal-800 border-b-[1px] border-neutral-300 w-full'>
          Education
        </h1>
            ...
            <div key={index} className='flex justify-between'>
              <div className='flex gap-[2px]'>
                <h1 className='font-bold'>
                  {item.course}
                  <span className={`${item.course === '' ? 'hidden' : ''}`}>,</span>
                </h1>
                <h1>{item.name}</h1>
              </div>
              <div className='flex gap-[2px]'>
                <h1>
                  {item.score}
                  <span className={`${item.score && item.duration ? '' : 'hidden'}`}> |</span>
                </h1>
                <h1>{item.duration}</h1>
              </div>
            </div>
            ...
            <div className='font-bold text-[10px] flex justify-between'>
              ...
              <div className='flex'>
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className={`text-teal-800 ${item.gitlink ? '' : 'hidden'}`}
                >
                  GitHub
                </a>
